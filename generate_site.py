...
def convert_name(name):
    return name.lower().replace(' ', '-').replace('---', '-').replace("'", '').replace('?', '').replace('(','').replace(')', '').replace('&-', '').replace(',', '')
    

# TODO: Support cross referencing somehow. 

# Find each xml file in faqxml
tree = ET.parse('faqxml/Wave_1/Characters.xml')
...
f = open(os.path.join(output_dir, "Wave_1/Characters/index" + ".html"), "w")
f.write(page)
f.close()
